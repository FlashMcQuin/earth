...
class vecteurs(list):
    ...
    def angles (self,other):
        return(np.arccos(self.Pscal(other)/(self.norme()*other.norme()))) #résultat en radians
...
class Point_Terre(vecteurs):
    ...
    def cartesien(self):
        ...
        self1=vecteurs([self2[0],self2[1]*(2*np.pi)/360,self2[2]*(2*np.pi)/360])
...
def affi():
    ...
    def clicked1():
        ...
        text1.configure(text="{} km".format(round(Point_Terre.distance(nouvp.cartesien(),nouvp1.cartesien())/10**3,2)))
        text2.configure(text="{} km".format(round(Point_Terre.distance(nouvp1.cartesien(),nouvp2.cartesien())/10**3,2)))
        ...
        f.write("latence V1-V2 (Paris-N.Y.C) sat = {} s \n".format(Point_Terre.latencesat(nouvpca1,nouvpca,PSAT)))
        f.write("latence V1-V2 (Paris-N.Y.C) fibr = {} s \n".format(Point_Terre.latencefibre(nouvpca1,nouvpca)))
        f.write("dist V1-V2 (Paris-N.Y.C) = {} km \n\n".format(round(Point_Terre.distance(nouvpca,nouvpca1)/10**3,2)))

        f.write("latence V1-V3 (Paris-Buenos aire) sat = {} s \n".format(Point_Terre.latencesat(nouvpca2,nouvpca,PSAT)))
        f.write("latence V1-V3 (Paris-Buenos aire) fibr = {} s \n".format(Point_Terre.latencefibre(nouvpca2,nouvpca)))
        f.write("dist V1-V3 (Paris-Buenos aire) = {} km".format(round(Point_Terre.distance(nouvpca,nouvpca2)/10**3,2)))
        f.close()
    ...
    btn1=tk.Button(fenetre,text="Calculer",bg="grey",fg="white",command=clicked1)
    btn1.grid(column=7,row=0)
    btn2=tk.Button(fenetre,text="Afficher",bg="grey",fg="white",command=clicked2)
    btn2.grid(column=7,row=1)
    btn3=tk.Button(fenetre,text="preset",bg="grey",fg="white",command=clicked3)
